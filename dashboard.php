...
['latestBtn','latestBtnDesktop'].forEach(id=>{$(id)&&$(id).addEventListener('click',()=>{const d=state.latestDate,from=new Date(new Date(d).getFullYear(),new Date(d).getMonth(),1).toISOString().slice(0,10);syncDateInputs(from,d);closeSheet();loadDashboard(true);startAutoRefresh()})});
['mtdBtn','mtdBtnDesktop'].forEach(id=>{$(id)&&$(id).addEventListener('click',()=>{const d=state.latestDate,from=new Date(new Date(d).getFullYear(),new Date(d).getMonth(),1).toISOString().slice(0,10);syncDateInputs(from,d);closeSheet();loadDashboard(true);startAutoRefresh()})});
...
applyPrefs();syncDateInputs('<?php echo h($dateFrom); ?>','<?php echo h($dateTo); ?>');setTab('overview');setMix('products');loadDashboard(false);startAutoRefresh();
</script>
</body>
</html>
